...
class UseMapper(BaseMapper):
    ...
    def get_model(self):
        ...
        if not os.path.exists(model_file):
            ...
            r = requests.get("https://storage.googleapis.com/tfhub-modules/google/universal-sentence-encoder/4.tar.gz", stream=True)
            zip_file = os.path.join(model_file, "4.tar.gz")

            # Save zipped model data to disk
            print("Streaming USE data now")
            with open(zip_file, 'wb') as f:
                f.write(r.raw.read())

            # Unzip model data
            print("Unzipping USE data now")
            tar = tarfile.open(zip_file, "r:gz")
            tar.extractall(path = model_file)
            tar.close()

            # Delete model zip data
            print("Removing USE zip file now")
            os.remove(zip_file)
